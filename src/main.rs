mod bytepair;
use crate::bytepair::{encode_u8, decode_u8, encode_u16, decode_u16, encode_u32, decode_u32, most_frequent_codepoint, substitute_pairs};
use std::vec::Vec;

fn main() {
    let text: &str = "A Programmer’s Introduction to Unicode
...
Unicode is a fascinating and complex system. It has a many-to-one mapping between bytes and code points, and on top of that a many-to-one (or, under some circumstances, many-to-many) mapping between code points and “characters”. It has oddball special cases in every corner. But no one ever claimed that representing all written languages was going to be easy, and it’s clear that we’re never going back to the bad old days of a patchwork of incompatible encodings.";
    ...
    // test encode decode
    let decoded_u8 = decode_u8(encode_u8(text));
    println!("{}", decoded_u8);
    let decoded_u16 = decode_u16(encode_u16(text));
    println!("{}", decoded_u16);
    let decoded_u32 = decode_u32(encode_u32(text));
    println!("{}", decoded_u32);

    let tokens: Vec<u8> = encode_u8(text);
    // Note: Not byte length (that stays the same)
    println!("Token length before: {}", tokens.len());

    let vocab_size: u16 = 276;
    let num_merges: u16 = vocab_size - 256;
    fn iterative_bytepair_encoding(num_merges: u16, tokens: &[u8]) -> Vec<u32> {
        let mut next_mint: u32 = 256;
        // first iteration (done once with original reference)
        let original_most_frequent = most_frequent_codepoint(tokens);
        let mut bytepaired_tokens: Vec<u32> = substitute_pairs(tokens, original_most_frequent, next_mint);
        next_mint += 1;

        for _ in 0..(num_merges - 1) {
            let most_frequent = most_frequent_codepoint(&bytepaired_tokens);
            bytepaired_tokens = substitute_pairs(&bytepaired_tokens, most_frequent, next_mint);
            next_mint += 1;
        }
        bytepaired_tokens
    }

    let result_tokens = iterative_bytepair_encoding(num_merges, &tokens);
    println!("Token length after {}", result_tokens.len());
    // Attention all you need: 6811 -> 5125
    // Programmer's guide to unicode: 24292 -> 19226
}
